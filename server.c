...
DWORD WINAPI ServiceClient(LPVOID userIP)
{
    ...
    while (1)
    {
        ...
        if (strcmp(buffer, "exit\n") == 0)
        {
            ...
            closesocket(user[myIP].newSockfd); // Close the socket for this client
            ...
            for (i = 0; i < userCount; i++)
            {
                if (i != myIP)
                {
                    ...
                    sprintf(exitMessage, "User %s has exited the chat.\n", user[myIP].username);
                    ret = send(user[i].newSockfd, exitMessage, strlen(exitMessage), 0);
                    if (ret < 0)
                    {
            ...
            strcat(user[myIP].username, " OFFLINE");
        ...
        if (buffer[0] == '@')
        {
            ...
            for (i = 0; i < userCount; i++)
            {
                ...
                if (strstr(buffer, uname))
                {
                    ...
                    sprintf(filename, "logs/ChatLog-%s_WITH_%s_ON_%d-%d-%2d_AT_%d-%d.txt", user[myIP].username, user[i].username, timeinfo->tm_mday, timeinfo->tm_mon + 1, timeinfo->tm_year + 1900, timeinfo->tm_hour, timeinfo->tm_min);
                    ...
                    // Write private message to log file
                    fprintf(logfile, "%s\n", privateMsg);
